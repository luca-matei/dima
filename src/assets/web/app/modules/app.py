class lmApp:
    ...
    def start(self):
        settings = util.read(self.app_dir + "settings.ast")
        self.settings = settings

        lm.lutil.level = settings.get("log_level")
        lm.lutil.reset()

        log("Loading settings ...")
        for s in ("lmid", "domain", "themes"):
        ...
        for s in self.db.execute("select id, name, parent from sections;"):
            for i in (0, 2):
                if s[i] not in util.get_keys(self.sections):
                    self.sections[s[i]] = {}
                    self.pages[s[i]] = {}
        ...
        for l in self.db.execute("select id, code from langs;"):
            ...
            for s in util.get_keys(self.sections):
                self.pages[s][l[0]] = {}
        ...
        for m in self.db.execute("select id, name from methods;"):
            ...
            for s in util.get_keys(self.sections):
                for l in [l_id for l_id in util.get_keys(self.langs) if isinstance(l_id, int)]:
                    self.pages[s][l][m[0]] = {}
